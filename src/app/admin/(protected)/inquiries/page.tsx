    ...
    <div className="space-y-4">
      ...
      <ContentListSearch
        ...
        items={inquiries.map((inquiry) => {
          let tone: 'default' | 'success' | 'warning' | 'info' | 'critical' = 'default'
          if (inquiry.status === 'new') tone = 'success'
          if (inquiry.status === 'contacted') tone = 'info'
          
          const date = inquiry.submittedAt ? new Date(inquiry.submittedAt).toLocaleDateString() : ''
          ...
          return {
            ...
            badges: [
              ...(inquiry.status ? [{ label: inquiry.status.toUpperCase(), tone }] : []),
              ...(inquiry.service ? [{ label: inquiry.service, tone: 'default' as const }] : []),
            ],
