...
export const getDashboardStats = async (): Promise<DashboardStats> => {
  ...
  const [services, posts, authors, jobs, legal, inquiries, drafts, media] = await Promise.all([
    client.fetch<number>('count(*[_type == "service" && !(_id in path("drafts.**"))])'),
    client.fetch<number>('count(*[_type == "post" && !(_id in path("drafts.**"))])'),
    client.fetch<number>('count(*[_type == "author" && !(_id in path("drafts.**"))])'),
  ...
  const recent = await client.fetch<DashboardStats['recent']>(
    `*[!(_id in path("drafts.**")) && _type in ["homePage","aboutPage","careersPage","contactPage","blogIndex","servicesIndex","service","post","job","legalPage","siteSettings","inquiry"]] | order(_updatedAt desc)[0...3]{
      _id,
      _type,
      _updatedAt,
...
export const listMediaAssets = async (search = '') => {
  ...
  return await client.fetch(
    `*[_type in ["sanity.imageAsset", "sanity.fileAsset"] ${filter}] | order(_updatedAt desc)[0...500]{
      _id,
      _type,
      originalFilename,
      url,
